...
MAX_QUESTIONS_PER_SESSION = 3  # New constant for question limit
...
class SessionState:
    def __init__(self):
        ...
        self.job_url = ""
        self.resume_input_method = "upload"
...
def main():
    ...
    state.name = st.sidebar.text_input("What is your name? :red[*]", placeholder="Elon Gates 🦹")
    ...
    state.resume_input_method = st.sidebar.radio(
        "Choose resume input method: :red[*]", 
        ("Upload Resume", "Paste your Resume"), 
        help="Upload your resume in PDF format or manually type your resume content. This helps our AI understand your qualifications and experience."
    )

    if state.resume_input_method == "Upload Resume":
        uploaded_resume = st.sidebar.file_uploader("Upload your Resume (PDF) :red[*]", type="pdf", label_visibility="collapsed")
        if uploaded_resume:
        ...
        state.resume_text = st.sidebar.text_area("Share more about yourself: :red[*]", placeholder="I am a PM with 5+ years of experience in B2B SaaS and FinTech. I have worked across.....", height=200)
    ...
    if st.sidebar.button("Start Chat"):
        if not state.name or not state.job_type or not state.resume_text or (state.job_input_method == "Enter Job Description" and not state.job_description_text) or (state.job_input_method == "Enter Job URL" and not state.job_url):
            ...
            if state.job_url:
                with st.spinner("Fetching job description from URL..."):
                    ...
                    if html_content:
                        ...
                        time.sleep(2)
                        st.toast("This might be due to strict scraping policies by the target company. Could you please paste the job description in the side box instead?")
            else:
...
def send_initial_messages(state, assistant_id):
    initial_prompt = f"""
    The candidate, {state.name}, is applying for Product Manager roles. Please review the resume and job description provided. Start by thanking, {state.name} ,for sharing these details. Mention that you've reviewed the information and highlight the following points(in bullets): Company Name, Role, Desired Years of Experience, Desired Skill Set, and Allignment (how the candidate's skills and experiences align with the job requirements). End with sharing that I can help you with personalized messaging for the job application. If the resume or job description is irrelevant or incomplete, kindly request a resubmission. Keep your response enthusaistic, conversational and concise and within 125 words. Here is the candidate's resume: {state.resume_text}. Here is the job description: {state.job_description_text}
    """
...
def chat_interface(state, assistant_id):
    ...
    "Cold Email | Hiring Manager": 
        "Based on the candidate resume and the provided job description, craft a cold email to the recruiter. The email should be under 300 words, mention three relevant pointers highlighting the candidate's skills and experiences, and express interest in the role. Share that the resume and a portfolio have been attached for reference.",
    "Cold Email | Hiring Manager": 
        "Based on the candidate resume and the provided job description, craft a cold email to the founder of the company. The email should be under 300 words, mention three relevant pointers highlighting the candidate's skills and experiences, and express interest in the role. Share that the resume and a portfolio have been attached for reference."
    }
